document.addEventListener('DOMContentLoaded', () => {
    ...
    socket.on('roomCreated', (roomId) => {
        ...
        const shareLinkP2 = document.getElementById('share-link-p2');
        const shareLinkSpectator = document.getElementById('share-link-spectator');
        if(shareLinkP2 && shareLinkSpectator && shareContainer && lobbyContent) {
    ...
    socket.on('promptRoll', ({ targetPlayerKey, text, action }) => {
        ...
        btn.classList.toggle('inactive', !isMyTurnToRoll && !isSpectator);
    });
    ...
    // O ouvinte de 'diceRoll' agora só toca o som
    socket.on('diceRoll', () => playRandomSound('dice')); 
