    ...
    function populateIngameSheet(fighter, isGmView = false) {
        ...
        const updateAllEquipment = (eventSource) => {
            ...
            const checkAndHandleRequirement = (itemName, itemType, itemSelect, defaultOption) => {
                ...
                const itemData = (GAME_RULES[itemType] || {})[itemBaseType] || {};
    ...
    function showGmInventoryEditor(player) {
        ...
        catalogContainer.querySelectorAll('.shop-tab-btn').forEach(btn => {
            btn.onclick = () => {
                ...
                // Dispara o evento de input para refiltrar os itens ao trocar de aba
        ...
        document.getElementById('gm-inventory-search').oninput = (e) => {
            ...
            // Busca apenas dentro da aba (grid) ativa
            const activeGrid = catalogContainer.querySelector('.shop-item-grid.active');
            if (activeGrid) {
                // Percorre apenas os cards da grid ativa
        ...
        // Garante que o estado inicial da busca seja aplicado corretamente
        document.getElementById('gm-inventory-search').dispatchEvent(new Event('input'));
...
chevron_right
Peço desculpas pelo erro! Você está absolutamente correto. Analisando novamente o meu código e a sua imagem, percebo que, embora eu tenha gerado a estrutura HTML correta com as abas e as grades separadas, eu falhei em adicionar o código JavaScript que de fato torna as abas interativas dentro da função `showGmInventoryEditor`. A lógica para "escutar" os cliques nos botões de categoria e alternar a visibilidade das grades de itens não foi portada da função da loja para a função do editor de inventário.

Foi um descuido da minha parte. A correção é exatamente o que você apontou: fazer os botões de categoria funcionarem para filtrar os itens exibidos.

Vou corrigir isso agora no `client.js`, adicionando os event listeners necessários para que as abas funcionem como esperado.

Aqui está o arquivo `client.js` corrigido.
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function showGmInventoryEditor(player) {
        ...
        const modalContent = document.getElementById('modal-content');
        const catalogContainer = modalContent.querySelector('#gm-item-catalog-container');
        ...
        if (catalogContainer) {
            catalogContainer.querySelectorAll('.shop-tab-btn').forEach(btn => {
                btn.onclick = () => {
                    ...
                    modalContent.querySelector('#gm-inventory-search').dispatchEvent(new Event('input'));
                };
            });
            ...
            catalogContainer.querySelectorAll('.gm-inv-item-card').forEach(card => {
                card.onclick = () => {
                    ...
                    if (itemData.isAmmunition) {
                         stagedAmmunition++;
                         modalContent.querySelector('#gm-ammo-input').value = stagedAmmunition;
                         return;
                    }
            ...
            const searchInput = modalContent.querySelector('#gm-inventory-search');
            if(searchInput) {
                ...
                // Dispara o evento uma vez para garantir o estado inicial correto
