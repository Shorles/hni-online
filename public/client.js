document.addEventListener('DOMContentLoaded', () => {
    ...
    function showDiceRollAnimation(playerKey, rollValue, diceType) {
        console.log(`[DIAGNÓSTICO] Função chamada para: playerKey=${playerKey}, rollValue=${rollValue}, diceType=${diceType}`);
        
        const diceOverlay = document.getElementById('dice-overlay');
        const diceContainer = document.getElementById(`${playerKey}-dice-result`);

        if (!diceOverlay || !diceContainer) {
            console.error('[DIAGNÓSTICO] ERRO: Elemento do dado ou overlay não encontrado no DOM!');
            return;
        }
        console.log(`[DIAGNÓSTICO] Elemento encontrado: #${diceContainer.id}`);
        
        const imagePrefix = (diceType === 'd3') ? (playerKey === 'player1' ? 'D3P-' : 'D3A-') : (playerKey === 'player1' ? 'diceP' : 'diceA');
        const imageUrl = `images/${imagePrefix}${rollValue}.png`;
        console.log(`[DIAGNÓSTICO] Tentando carregar a imagem: ${imageUrl}`);
        
        diceContainer.style.backgroundImage = `url('${imageUrl}')`;
        ...
        setTimeout(hideAndResolve, 3000); 
    }
