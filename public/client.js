document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderCombatants(state) {
        ...
        Object.values(state.combatants.party).forEach((p, index) => {
            partyContainer.appendChild(createCombatantElement(p, state, index));
        });

        Object.values(state.combatants.enemies).forEach((e, index) => {
            const enemyEl = createCombatantElement(e, state, index);
            if (e.hp > 0) {
                enemyEl.addEventListener('click', () => selectTarget(enemyEl, e.id));
            }
        ...
        if (isGm && state.phase === 'initiative_roll') {
            ...
            if (hasEnemyUnrolled) {
                ...
                // <<< CORREÇÃO: Adicionando o event listener que faltava
                gmRollBtn.onclick = () => {
        ...
        if (state.whoseTurn) {
            document.querySelector(`.combatant-sprite-wrapper[data-id="${state.whoseTurn}"]`)?.classList.add('active-turn');
        }
        if (selectedTargetId) {
             document.querySelector(`.combatant-sprite-wrapper[data-id="${selectedTargetId}"]`)?.classList.add('selected-target');
        }
    }

    function createCombatantElement(c, state, index) {
        const wrapper = document.createElement('div');
        wrapper.className = 'combatant-sprite-wrapper';
        wrapper.dataset.id = c.id;
        wrapper.style.setProperty('--char-index', index);

        if (c.hp <= 0) {
            wrapper.classList.add('defeated');
        }
        ...
        wrapper.innerHTML = `
            <div class="combatant-hud">
                <div class="name-plate">${c.nome}</div>
                <div class="hp-bar-container"><div class="hp-bar" style="width: ${hpPercentage}%"></div></div>
                <div class="hp-text">${c.hp} / ${c.hpMax}</div>
            </div>
            <img src="${c.img}" class="fighter-img ${c.isPlayer ? '' : 'enemy-sprite'}" />
            <div class="initiative-roll-area">${initiativeButton}</div>
        `;
        
        if(initiativeButton) {
            wrapper.querySelector('.roll-init-btn').onclick = (e) => {
                e.stopPropagation(); // Impede que o clique selecione o alvo
