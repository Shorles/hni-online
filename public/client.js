...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateUI(state) {
        ...
        if (myPlayerKey && state.fighters.players[myPlayerKey] && state.fighters.players[myPlayerKey].status === 'active') {
            ...
            actionButtonsWrapper.innerHTML = `<button class="action-btn" data-action="attack" ${!canAct ? 'disabled' : ''}>Atacar</button><button class="end-turn-btn" data-action="end_turn" ${!canAct ? 'disabled' : ''}>Passar Turno</button>`;
        }
    }
