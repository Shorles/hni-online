...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function showGmInventoryEditor(player) {
        ...
        const allItemsMap = new Map(allGameItemsFlat.map(item => item.name ? [item.name, item] : [null, null]).filter(entry => entry[0]));
        ...
        const buildItemCatalogHtml = () => {
            ...
            Object.entries(allGameItemsCategorized).forEach(([category, items], i) => {
                ...
                catalogHtml += `<div class="gm-inventory-grid shop-item-grid ${i === 0 ? 'active' : ''}" id="catalog-grid-${categoryId}">`;
                    ...
                    let imgPath = item.img;
                     if (!imgPath) {
                        if (item.type === 'armor') {
                            ...
                            imgPath = `/images/armas/${armorImgName}.png`;
                        } else if (item.type === 'shield') {
                            const shieldImgName = item.name === 'Médio' ? 'Escudo Medio' : `Escudo ${item.name}`;
                            imgPath = `/images/armas/${shieldImgName}.png`;
                        }
                    }
                    catalogHtml += `
