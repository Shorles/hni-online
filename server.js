...
function createNewAdventureState(lobbyState) {
    const adventureState = {
        ...
        phase: 'npc_setup', // CORRIGIDO: Inicia na tela de seleção de NPCs
...
function startBattle(state) {
    ...
    logMessage(state, `É a vez de ${firstFighter.nome}.`, 'turn');
}
...
io.on('connection', (socket) => {
    ...
    socket.emit('initialData', { 
        characters: { npcs: Object.keys(ALL_NPCS).map(name => ({ name, img: `images/lutadores/${name}.png` })) }, 
        spells: ALL_SPELLS, scenarios: ALL_SCENARIOS 
    });
