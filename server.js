...
const io = new Serve//  VERSÃO CORRIGIDA E ROBUSTA DO SERVER.JS
...
// ... (as outras funções como executeAttack, endTurn, etc. permanecem as mesmas) ...
...
// ... (fim das funções de lógica de jogo) ...

// <<< NOVA FUNÇÃO DE VALIDAÇÃO, A PROVA DE FALHAS >>>
...
io.on('connection', (socket) => {
    ...
    socket.on('playerAction', (action) => {
        ...
        if (!roomId || !games[roomId] || !action || !action.playerKey) return; // Checagem básica
        
        const room = games[roomId];
        const state = room.state;

        // <<< USANDO A NOVA FUNÇÃO DE VALIDAÇÃO ROBUSTA >>>
        if (!isActionValid(state, action)) {
            console.log(`Ação inválida recebida e rejeitada: `, action, `na fase: ${state.phase}`);
            return; // Ação inválida, encerra a execução aqui para evitar o travamento.
        }
        
        // Se a ação for válida, o código prossegue normalmente.
        ...
        switch (action.type) {
            ...
            case 'request_get_up':
                ...
                // A validação principal já foi feita, mas uma checagem extra não machuca
...
server.listen(PORT, () => console.log(`Servidor rodando na porta ${PORT}`));r(server);
